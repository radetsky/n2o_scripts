...
//        panel.hide();
    ...
    function render() {
      ...
      //var ul = $("<div/>").attr({'class': 'btn-toolbar'}).appendTo(panel);
      var ul = panel.attr({'class': 'btn-toolbar'});
      var events = [];
      createToolbar().map(function(i){
        $.extend(i, {postRender: function(){}, renderHtml: function(){
          ...
          var b = $("<button>").attr({
            ...
            'data-toggle': self.settings.toggle,
            class: 'btn '+ self.settings.class,
            value: self._text ? self.encode(self._text) : '',
            role: 'presentation'});
          if(icon) $("<i/>").attr('class', icon).addClass('icon-large').appendTo(b);

          return b.clone().wrap("<div/>").parent().html();
        }}).renderTo($(ul).get(0)); //$("<li>").appendTo(ul).get(0)
      ...
      DOM.removeClass(editor.getBody(), 'mce-edit-focus');
      $(editor.getBody()).attr('data-html', true);
    }
